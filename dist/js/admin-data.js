...
var SUBSCRIBER_STATUSES = [{
  ...
  color: '#3B82F6',
  bg: '#DBEAFE',
  desc: 'Signed up — call to discuss and request payment'
}, {
  value: 'Payment Made',
  label: 'Payment Made',
  color: '#F59E0B',
  bg: '#FEF3C7',
  desc: 'Payment received — confirm with chef that delivery is possible'
}, {
  value: 'Active Deliveries',
  label: 'Active Deliveries',
  color: '#3A813D',
  bg: '#D4EDDA',
  desc: 'Deliveries underway'
}, {
  value: 'Paused Deliveries',
  label: 'Paused Deliveries',
  color: '#9CA3AF',
  bg: '#F4F4F4',
  desc: 'Temporarily paused by subscriber'
}, {
  value: 'Deactivated',
  label: 'Deactivated',
  color: '#D0342C',
...
var defaultContent = {
  ...
  how_ch1_desc: "Submit your application with your cuisine style and sample menus. Our team reviews your background to ensure Home Meal standards are met.",
...
// ── Chefs ──
function loadChefs() {
  return _apiGet('/api/chefs');
}
function saveChefs() {/* no-op: saves go through updateChef */}
function updateChef(chef) {
  return _apiPut('/api/chefs/' + chef.chef_id, chef);
}
function addChef(chef) {
  return _apiPost('/api/chefs', chef);
}
function deleteChef(chef_id) {
  return _apiDelete('/api/chefs/' + chef_id, {});
}

// ── Content ──
function loadContent() {
  return _apiGet('/api/content');
}
function saveContent(c) {
  return _apiPut('/api/content', c);
}

// ── Subscribers ──
function loadSubscribers() {
  return _apiGet('/api/subscribers');
}
function saveSubscribers() {/* no-op */}
function updateSubscriber(sub) {
  return _apiPut('/api/subscribers/' + sub.id, sub);
}
function addSubscriberAPI(sub) {
  return _apiPost('/api/subscribers', sub);
}
function deleteSubscriber(id) {
  return _apiDelete('/api/subscribers/' + id, {});
}

// ── Applications ──
function loadApplications() {
  return _apiGet('/api/applications');
}
function saveApplications() {/* no-op */}
function updateApplication(app) {
  return _apiPut('/api/applications/' + app.id, app);
}
function deleteApplication(id, body) {
  return _apiDelete('/api/applications/' + id, body || {});
}

// ── Notifications ──
function loadNotifications() {
  return _apiGet('/api/notifications');
}
function saveNotifications() {/* no-op */}
function pushNotification(type, message, ref_id) {
  return _apiPost('/api/notifications', {
    type: type,
    message: message,
    ref_id: ref_id || null
  });
...
function markNotificationRead(id) {
  return _apiPut('/api/notifications', {
    id: id
  });
}
...
function loadPendingMenus() {
  return _apiGet('/api/menus');
}
function savePendingMenus() {/* no-op */}
function addMenu(entry) {
  return _apiPost('/api/menus', entry);
}
function updateMenu(id, status, reason) {
  return _apiPut('/api/menus', {
    id: id,
    status: status,
...
// ── Chef Accounts ──
function loadChefAccounts() {
  return _apiGet('/api/accounts');
}
function saveChefAccounts() {/* no-op */}
function upsertChefAccount(acct) {
  return _apiPost('/api/accounts', acct);
}
function deleteChefAccount(chef_id) {
  return _apiDelete('/api/accounts', {
    chef_id: chef_id
  });
}
